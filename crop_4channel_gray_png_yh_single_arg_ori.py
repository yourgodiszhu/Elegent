...
class ImageProcessor:
    def __init__(self, window_size=(2048, 2048), overlap=(400, 400)):
        ...
        try:
            ...
            for y in range(0, h, stride_h):
                for x in range(0, w, stride_w):
                    ...
                    output_path = os.path.join(output_dir, f'patch_y{y}_x{x}.png')
                    ...
                    if os.path.basename(current_path)==first:
                        ...
                        cv2.imwrite(os.path.join(crop_center_path,file_name),center_patch)
...
def process_single_category(cls_all_path, thread_per_process=4):
    """处理一个cls_all分类（进程级任务）"""
...
def main_proc(params, process_workers=4, thread_per_process=4):
    """主处理函数：进程池+线程池"""
    # 获取所有需要处理的分类路径
    ...
    with Pool(processes=process_workers) as pool:
        # 使用partial固定线程数参数
        pool.map(partial(process_single_category, thread_per_process=thread_per_process), all_tasks)
