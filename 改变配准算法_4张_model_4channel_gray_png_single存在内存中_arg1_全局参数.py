...
def process_subdir(subdir_path, H_dict, HEIGHT, WIDTH, merged_memory ):
    black_image = np.full((2048, 2048), 0, dtype=np.uint8)
    # print(subdir_path)
    # print(merged_memory)
    if subdir_path in merged_memory:
    ...
    print(f"线程 {os.getpid()} - {threading.current_thread().name} 正在处理子目录: {subdir_path}",H_dict)
    out_path=''
    for i in H_dict:
...
def process_directory(dir_path, images_path, HEIGHT, WIDTH, first_dir,matcher):
    ...
    merged_memory = manager.dict()  # key: subdir_path, value: merged_image（np.ndarray）
    ...
    # print(images_path,max_col,max_row)
    # 主处理循环
    for idx, i in enumerate(tqdm.tqdm(images_path)):
                    ...
                    if match:
                        ...
                        if f"1_{b - 1}_{c}.png" in patch_overlab_dict:
                            ...
                            if not is_mostly_black(diff):
                                ...
                                # cv2.imwrite(rf'D:\3d\temp_ceshi_name\{os.path.basename(i).replace(".png","_right.png")}',img1)
                                # cv2.imwrite(rf'D:\3d\temp_ceshi_name\{f"1_{b - 1}_{c}_left.png"}',img2)
        ...
        # print(out_path,registered_color)
        # cv2.imwrite(os.path.join(out_path, os.path.basename(i)), registered_color)
        # print("执行前", i)
        # h, w = registered_color.shape[:2]  # 获取高和宽（单通道或多通道）
        # size = 1848
    ...
    for f in all_threads:
        f.result()
    # time.sleep(30)
    keep_vars = ['dir_path', 'first_dir','subdirs','out_path','error_patchs','merged_image','merged_memory']
    ...
    fixed_merged_image = cv2.imread(
        os.path.join(dir_path.replace(os.path.basename(dir_path), first_dir), 'DAPI', fr'{first_dir}_DAPI_out1.png'),
        cv2.IMREAD_GRAYSCALE)
    # 初始化
    merged_image_ori = cv2.imread(
        os.path.join(dir_path, 'DAPI', fr'{os.path.basename(dir_path)}_DAPI_merged_image_moving.png'),
        cv2.IMREAD_GRAYSCALE)
    merged_memory_ori = manager.dict()
    for subdir_path in subdirs:
        # if subdir_path not in merged_memory_ori:
        img = cv2.imread(os.path.join(subdir_path,
                                      fr'{subdir_path.split(os.sep)[-2]}_{subdir_path.split(os.sep)[-1]}_merged_image_moving.png'),
                         cv2.IMREAD_GRAYSCALE)
    ...
    import re
    # if 'extractor' in locals():  # 检查变量是否存在
    #     del extractor
        # import gc
        # gc.collect()

        # if 'matcher_lightglue' in locals():
        #
        #     del matcher_lightglue
        #     import gc
        #     gc.collect()
        # if 'extractor1' in locals():
        #     del extractor1
        # if 'matcher1' in locals():
        #     del matcher1
    # 强制同步并重置CUDA上下文
    # torch.cuda.synchronize()
    # torch.cuda.reset_peak_memory_stats()  # 可选：重置内存统计
    # torch.cuda.synchronize()
    # torch.cuda.empty_cache()
    # 释放TensorRT上下文
    # torch.cuda.synchronize()  # 让PyTorch接管CUDA

    extractor1 = TRTInference("aliked-n16512.trt", "aliked-n16", trt.Logger(trt.Logger.ERROR))

    for g in result:
        torch.cuda.empty_cache()  # 每次循环前清理缓存
        coords = []
        s=time.time()
        for i in g:
        ...
        new_start_y = max(0, start_y - (target_h - h) // 2)
        new_start_x = max(0, start_x - (target_w - w) // 2)
        new_end_y = min(HEIGHT, new_start_y + target_h)
        new_end_x = min(WIDTH, new_start_x + target_w)
        ...
        print('模型开始')
        # time.sleep(1)
        # extractor1 = ALIKED2048().eval().half().to(device)  # load the extractor
        #
        # matcher1 = LightGlue(features='aliked').eval().half().to(device)  # load the matcher


        # if not extractor1.context:
        #     raise RuntimeError("TRT engine failed to initialize")
        # matcher1 = LightGlue(features='aliked', filter_threshold=0.05).eval().to(device)
        print('lightglue加载完成')
        ...
        # H, angle_deg, end_flag=auto_register(expanded_fixed_img,expanded_moving_img,device, extractor1, matcher1)
        # if expanded_fixed_img.shape[0]>10000:
        #     continue
        # torch.cuda.empty_cache()
        print('前处理',time.time()-s)
        ...
        try:
            ...
            moving_img_end = cv2.warpPerspective(expanded_moving_img, H, (expanded_moving_img.shape[1], expanded_moving_img.shape[0]))
            print('变换结束')
            # print('光流之前')
            # torch.cuda.empty_cache()
            s2=time.time()
