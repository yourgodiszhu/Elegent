...
def process_group(g, dir_path, first_dir, subdirs, HEIGHT, WIDTH,
    ...
    try:
        ...
        offset_y0 = max(0, -new_start_y)
        offset_x0 = max(0, -new_start_x)
        offset_y1 = max(0, new_end_y - HEIGHT)
        offset_x1 = max(0, new_end_x - WIDTH)
        new_start_y = max(0, new_start_y)
        new_start_x = max(0, new_start_x)
        new_end_y = min(HEIGHT, new_end_y)
        new_end_x = min(WIDTH, new_end_x)
        # if new_start_y < 0:
        #     new_end_y -= new_start_y  # 往下补偿
        #     new_start_y = 0
        # if new_start_x < 0:
        #     new_end_x -= new_start_x  # 往右补偿
        #     new_start_x = 0
        # if new_end_y > HEIGHT:
        #     new_start_y -= (new_end_y - HEIGHT)  # 往上移
        #     new_end_y = HEIGHT
        # if new_end_x > WIDTH:
        #     new_start_x -= (new_end_x - WIDTH)  # 往左移
        #     new_end_x = WIDTH
        ...
        print('start_y-(target_h-h)//2',start_y-(target_h-h)//2)
        print('start_x-(target_w-w)//2',start_x-(target_w-w)//2)
        print('new_start_y',new_start_y+target_h)
        print('new_start_x',new_start_x+target_w)
        ...
        try:
            ...
            moving_img_end = moving_img_end[offset_y0:target_h - offset_y1,
