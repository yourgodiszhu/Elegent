...
class Image_Registration:
    ...
    def gen_pyramid_tiff_for_fluorescence(self, save_svs_path=''):
        ...
        kwargs_main = dict(
            ...
            compressionargs={'level': 100},  # 压缩级别80
            dtype=np.uint8,
            metadata=None
        )
        kwargs_jpeg = dict(
...
def main(params):
    ...
    #         with open(os.path.join(txt_path, txt_file), 'r',encoding='ignore') as f:
    #             content = f.read()
    ...
    #     return f"{zb}"
    # 定义全局字典用于存储读取的图像
    def rename_channel_new(key,config):
        name,channel=key.rsplit('_', 1)
        for filename in config:
            if os.path.basename(filename['img_path']) == name:
                return filename['default_name'][channel]
    ...
    def load_image(img_path):
        ...
        :return: 图像文件名和图像数据
        """
        img = universal_imread(os.path.join(png_dir, img_path))
        ...
        if img is not None:
            ...
            print(f"Loaded: {img_path}, shape=({H}, {W})")
            if key.endswith('.png'):
                key=key.split('.png')[0]
            return key, img
        else:
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        ...
        for future in as_completed(futures):
            ...
            if key is not None and img is not None:
                gray_images[key] = img
    ...
    background_image = np.memmap('temp_mmap.dat', dtype=np.uint8, mode='w+', shape=(N, H, W, 3))

    print(background_image.shape)
    # for i in range(4):
    #     if i ==0:
    #         background_image[i, :, :, 0] = DAPI_background_image
    #         Filter_params.append({'Filter_name': 'AR391_1_CY5', 'Exposure_time': '5',
    #                        'Excitation_intensity': '100', 'Digital_gain': '3',
    #                        'Number_of_focus_level': None, 'Focus_step_size': None,
    #                        'Bit_depth_per_channel': '8'})
    #         background_image[i, :, :, 1] = gray_images[0]
    #         background_image[i, :, :, 2] = gray_images[1]
    #     if 0<i<=3 :
    #         background_image[i, :, :, 0] = gray_images[i*3-1]
    #         background_image[i, :, :, 1] = gray_images[i*3]
    #         background_image[i, :, :, 2] = gray_images[i*3+1]
    #     if i ==4:
    #         background_image[i, :, :, 0] = gray_images[i*3-1]
    #         background_image[i, :, :, 1] = 0
    #         background_image[i, :, :, 2] = 0

    i = 0
 #    groups=[('AR391_2_SpOrange', 'AR391_3_SpOrange', 'AR391_4_SPOrange',),
 # (None, None, 'Maximum_DAPI.png'),
 # ('AR391_2_CY5', 'AR391_1_SpGreen', 'AR391_1_CY5'),
 # ('AR391_3_CY5', 'AR391_4_CY5', 'AR391_2_SpGreen'),
 #            ('AR391_3_SpGreen', 'AR391_4_SpGreen', 'AR391_1_SpOrange'),
    ...
    for group in groups:
        ...
        for key in group:
            ...
            if key is not None:
                background_image[i, :, :, c] = gray_images[key]
                Filter_params.append({
                    # 'Filter_name': 'DAPI' if key =='DAPI' else rename_channel(key, data_dict),
                    'Filter_name': 'DAPI' if key == 'DAPI' else rename_channel_new(key, config),
                    # 'Filter_name':key,
                    'Exposure_time': '5',
            ...
            c += 1
        i += 1
    # for group in zip_longest(*[iter(keys)] * 3):
    #     print("当前组:", group)
    #     if None in group:
    #         break
    #     c = 0
    ...
    #         if key is not None:
    #             background_image[i, :, :, c] = gray_images[key]
    #             Filter_params.append({'Filter_name': key, 'Exposure_time': '5',
    #                                   'Excitation_intensity': '100', 'Digital_gain': '3',
    #                                   'Number_of_focus_level': None, 'Focus_step_size': None,
    #                                   'Bit_depth_per_channel': '8'})
    #         if key is None:
    #             background_image[i, :, :, c] = 0
    #             Filter_params.append({'Filter_name': None, 'Exposure_time': '5',
    #                                   'Excitation_intensity': '100', 'Digital_gain': '3',
    #                                   'Number_of_focus_level': None, 'Focus_step_size': None,
    #                                   'Bit_depth_per_channel': '8'})
    #
    #         c += 1
            # background_image[i, :, :, 1] = gray_images[key]
            # Filter_params.append({'Filter_name': key, 'Exposure_time': '5',
            #         'Excitation_intensity': '100', 'Digital_gain': '3',
            #         'Number_of_focus_level': None, 'Focus_step_size': None,
            #                                     'Bit_depth_per_channel': '8'})

            # background_image[i, :, :, 2] = gray_images[key]
            # Filter_params.append({'Filter_name': key, 'Exposure_time': '5',
            #         'Excitation_intensity': '100', 'Digital_gain': '3',
            #         'Number_of_focus_level': None, 'Focus_step_size': None,
            #                                     'Bit_depth_per_channel': '8'})
        ...
        # i += 1
        # print(f"键: {key}, 值: {gray_images[key]}")
        # print("-" * 20)
    # for i in range(4):
    #     background_image[i, :, :, 0] = DAPI_background_image
    #     Filter_params.append({'Filter_name': 'DAPI', 'Exposure_time': '5',
    #                 'Excitation_intensity': '100', 'Digital_gain': '3',
    #                 'Number_of_focus_level': None, 'Focus_step_size': None,
    #                 'Bit_depth_per_channel': '8'})
    #     background_image[i, :, :, 1] = gray_images[0]
    ...
    #     background_image[i, :, :, 1] = 0
    #     background_image[i, :, :, 2] = 0
    # cv2.imwrite('pyramid_output/test.png', background_image)
    # gen_pyramid_tiff(save_svs=save_svs, background_image=background_image, label_path=label_path,
    ...
    save_svs_end = params['save_svs_dir']
    save_svs ='1.svs'
    print(desc)
    ...
    os.rename(save_svs,save_svs_end)
