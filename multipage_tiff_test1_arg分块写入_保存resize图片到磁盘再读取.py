...
def main(params):
    ...
    merge_speed=params['merge_speed']
    ...
    multi_hw = [[bk_h, bk_w], [bk_h // 2, bk_w // 2], [bk_h // 4, bk_w // 4], [bk_h // 8, bk_w // 8],
                [bk_h // 16, bk_w // 16],[bk_h // 32, bk_w // 32],[bk_h // 64, bk_w // 64]]

    def read_gray_image(png_dir, key,multi_hw,save_svs):
        """读取单张灰度图"""
        if key is None:
            return None, None
        if key=='DAPI':
        ...
        for size in multi_hw:
            ...
            if img is not None:
                if x >= 1:
                    ...
                    os.makedirs(fr'{save_svs.replace(".svs", "")}', exist_ok=True)
                    cv_imwrite_unicode(fr'{save_svs.replace(".svs", "")}/{key}_w{img.shape[1]}h{img.shape[0]}.png', img)
                print(rf'resize:{size}-写入耗时:', time.time() - s)
                x += 1
        return key, img_copy

    def load_gray_images(png_dir, keys, multi_hw,save_svs,num_threads=12):
        gray_images = {}
        # print(png_dir,keys)
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(read_gray_image, png_dir, key,multi_hw,save_svs)
                for key in keys
            ]
    ...
    try:
        with tifffile.TiffWriter(save_svs, bigtiff=True) as tif:
            ...
            svs_desc += f"|METADATA_JSON = {metadata_json}"
            print(svs_desc)
            groups = regroup(groups, mode=merge_speed)
            for i, hw in enumerate(multi_hw):
                if i>=1:
                    background_image=cv2.resize(background_image, (background_image.shape[1]//2,background_image.shape[0]//2), interpolation=cv2.INTER_LANCZOS4)
                for group in groups:
                    print("处理组:", group)
                    if i == 0:
                        s=time.time()
                        gray_images = load_gray_images(png_dir, group,multi_hw, save_svs,num_threads=12)
                        print('读取耗时',time.time()-s)
                        # print(gray_images)
                        s=time.time()
                        for no_x in range(merge_speed):
                            c = 0

                            for key in group[3*no_x:3*(no_x+1)]:
                                if key is not None:
                                    background_image[ :, :, c] = gray_images[key]
                                else:
                                    continue
                                c += 1
                            print('每一group耗时',time.time()-s)
                            s=time.time()
                        ...
                        for no_x in range(merge_speed):
                            ...
                            for key in group[3*no_x:3*(no_x+1)]:
                                if key is not None:
                                    background_image[:, :, c] = universal_imread(fr'{save_svs.replace(".svs","")}/{key}_w{hw[1]}h{hw[0]}.png')
                                else:
                                    background_image[:, :, c] = 0
                                    continue
                                c += 1
                            background_image_1 =background_image
    ...
    # print_slide_info(r"F:\3d\version3_6\jiechangCa.svs")