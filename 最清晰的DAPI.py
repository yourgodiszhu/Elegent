...
img_path=r'D:\3d\version3_4\cell2_Gray_date\cell2'

extractor = TRTInference("aliked-n16512.trt", "aliked-n16", trt.Logger(trt.Logger.ERROR))
x={}
y={}
count=0
for i in os.listdir(r'D:\3d\version3_4\cell2_Gray_date\cell2\cell2_1\DAPI\crop_moving_img'):
    img1_path=os.path.join(r'D:\3d\version3_4\cell2_Gray_date\cell2\cell2_1\DAPI\crop_moving_img',i)
    ...
    if (feats0['keypoint_scores'] > 0.8).sum().item() >= 100:
        ...
        x[1]=qxd
        for j in range(2,3):
            img1_path = os.path.join(rf'D:\3d\version3_4\cell2_Gray_date\cell2\cell2_{j}\DAPI\crop_moving_img', i)
            fixed1 = cv2.imread(img1_path, cv2.IMREAD_GRAYSCALE)
            feats0 = extractor.run(cv2.resize(fixed1, (512, 512)))
            qxd = do_cul_fov_iqa_fast(fixed1)
...
# for i in range(1,5):
#     shutil.copy(rf"D:\3d\version3_4\cell{i}_Gray_date\cell{i}\cell{i}_1\DAPI\cell{i}_1_DAPI_out1.png",rf"D:\3d\version3_4\all_out_png_cell{i}\Maximum_DAPI.png")
# print(y)
