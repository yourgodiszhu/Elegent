...
def get_img_path(params):
    # params = {}
    # params['data_dir'] = r'D:\3d\version3_4\cell2_Gray_date\cell2'
    ...
    # 获取A目录下所有子目录
    listdir_img_path = [d for d in sorted(os.listdir(img_path), key=extract_round_num)
    ...
    for i in os.listdir(os.path.join(img_path,listdir_img_path[0],'DAPI','crop_moving_img')):
        img1_path=os.path.join(img_path,listdir_img_path[0],'DAPI','crop_moving_img',i)
        fixed1 = universal_imread(img1_path)
        # print(fixed1.shape)
        ...
        if (feats0['keypoint_scores'] > 0.8).sum().item() >= 100:
            ...
            max_s.append(qxd)
            x[1]=qxd
            for z in range(2,len(listdir_img_path)):
                img1_path = os.path.join(img_path, listdir_img_path[z-1], 'DAPI', 'crop_moving_img', i)
                fixed1 = universal_imread(img1_path)
                # feats0 = extractor.run(cv2.resize(fixed1, (512, 512)))
                qxd = do_cul_fov_iqa_fast(fixed1)

                x[z] = qxd
            max_key = listdir_img_path[max(x, key=lambda k: x[k])]
            if max_key not in y :
                y[max_key]=1
            else:
                y[max_key]+=1
        ...
        if count>=3:
            # shutil.copy(fr"{params['save_dir']}/{max(y, key=lambda k: y[k])}_DAPI_out1.png",fr"{params['save_dir']}/DAPI.png")
            shutil.copy(fr"{params['save_dir']}/{listdir_img_path[0]}_DAPI_out1.png",fr"{params['save_dir']}/DAPI.png")

            print(f"Best DAPI image is {listdir_img_path[0]}_DAPI_out1.png")

            return fr'{max(y, key=lambda k: y[k])}_DAPI_out1.png'
