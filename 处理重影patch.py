...
x=['HC_2.npz','HC_3.npz','HC_4.npz']
s_start=time.time()
data = np.load(x[0],allow_pickle=True)
for i in data:
    print(data[i])
dir_path, first_dir, subdirs, out_path, error_patchs=data['dir_path'], data['first_dir'], data['subdirs'], data['out_path'], data['error_patchs']
dir_path = str(dir_path)             # -> str
first_dir = str(first_dir)           # -> str
subdirs = list(subdirs)              # -> list
...
merged_image=cv2.imread(os.path.join(dir_path, 'DAPI', fr'{os.path.basename(dir_path)}_DAPI_out1.png'),cv2.IMREAD_GRAYSCALE)
for subdir_path in subdirs:
        ...
        # if subdir_path not in merged_memory_ori:
        img = cv2.imread(os.path.join(subdir_path,
                                      fr'{subdir_path.split(os.sep)[-2]}_{subdir_path.split(os.sep)[-1]}_out1.png'),
                         cv2.IMREAD_GRAYSCALE)
...
print('主进程写图成功',time.time()-s_start)