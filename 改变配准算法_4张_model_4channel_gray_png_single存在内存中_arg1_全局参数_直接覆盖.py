...
def Loftr_pt(
    ...
    # print('这里没有问题2')
    s=time.time()
    mkpts0 = batch['mkpts0_f'].cpu().numpy() * (fixed_path.shape[0]//512)
    mkpts1 = batch['mkpts1_f'].cpu().numpy() * (fixed_path.shape[0]//512)
...
def process_subdir(subdir_path, H_dict, HEIGHT, WIDTH, merged_memory,first_dir,window_size,over_lab):
    black_image = np.full((window_size[0], window_size[0]), 0, dtype=np.uint8)
    # print(subdir_path)
    # print(merged_memory)
    if subdir_path in merged_memory:
        merged_image = merged_memory[subdir_path]
    ...
    for i in H_dict:
        ...
        moving_img = universal_imread(os.path.join(subdir_path, 'crop_moving_img', i))
        # print(H_dict)
        # print(H_dict[i])
        if H_dict[i] is None:
            # print('H_dict[i] is None')
        ...
        if match:
            ...
            # print('a',a,'b',b,'c',c)
            patch_y = int(c) * (window_size[0] - over_lab[0])
            patch_x = int(b) * (window_size[0] - over_lab[0])
            h, w = registered_color.shape[:2]  # 获取高和宽（单通道或多通道）
            size = window_size[0] - (over_lab[0] - 200)
            ...
            # a1=1
            try:
                merged_image[patch_y + (over_lab[0]-200)//2:end_y + (over_lab[0]-200)//2, patch_x + (over_lab[0]-200)//2:end_x + (over_lab[0]-200)//2] = patch
            except:
                merged_image[patch_y + (over_lab[0]-200)//2:end_y + (over_lab[0]-200)//2, patch_x + (over_lab[0]-200)//2:end_x + (over_lab[0]-200)//2] = 0
...
def is_mostly_black(img, threshold=10, ratio=0.95):
    """
...
def process_directory(dir_path, images_path, HEIGHT, WIDTH, first_dir,matcher,window_size,over_lab,config):
    # 声明使用全局变量
    ...
    merged_image = np.zeros((HEIGHT, WIDTH), dtype=np.uint8)
    # merged_image = np.full((HEIGHT, WIDTH), dtype=np.uint8, fill_value=127)
    error_img = np.full((window_size[0], window_size[0]), 0, dtype=np.uint8)
    ...
    def process_subdirs_with_dict(subdirs, H_dict_chunk, HEIGHT, WIDTH, merged_memory,first_dir):
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            executor.map(process_subdir, subdirs, repeat(H_dict_chunk), repeat(HEIGHT), repeat(WIDTH), repeat(merged_memory),repeat(first_dir),repeat(window_size),repeat(over_lab)
                         )
    ...
    for filename in config:
        ...
        # print(p1_val)
        if os.path.basename(filename['img_path']) ==dir_path.split("\\")[-1] :
          # 仅替换第2段:

            black, gamma, white = filename['color_info']['DAPI'][
                'black'
            ], filename['color_info']['DAPI']['gamma'], filename['color_info']['DAPI']['white']
            if int(black) == 0 and int(white) == 255 and int(gamma) == 1:
                flag1 = False
            else:
                lut = precompute_lut(black, white, gamma)
                flag1 = True
        if os.path.basename(filename['img_path']) ==first_dir :
          # 仅替换第2段:
            ...
            if int(black) == 0 and int(white) == 255 and int(gamma) == 1:
                ...
                lut_first = precompute_lut(black, white, gamma)
                flag1_first = True
    ...
    for idx, i in enumerate(tqdm.tqdm(images_path)):
        ...
        s=time.time()
        fixed_img = universal_imread(i)
        if flag1_first:
            fixed_img=cv2.LUT(fixed_img,lut_first)
        else:
            fixed_img=fixed_img
        # fixed_img=clahe.apply(fixed_img)
        parts = i.split("\\")
        parts[1] = dir_path.split("\\")[-1]  # 仅替换第2段

        # 重新组合
        new_path = "\\".join(parts)
        moving_img_ori = universal_imread(new_path)
        if flag1:
            moving_img = cv2.LUT(moving_img_ori, lut)  # 🔥 C 级并行加速操作
        else:
            moving_img = moving_img_ori
        # moving_img = histogram_matching(moving_img_ori, fixed_img)
        # moving_img = cv2.imread(i.replace(i.split("\\")[1], dir_path.split("\\")[-1]), cv2.IMREAD_GRAYSCALE)
        # moving_img_ori=moving_img
        # moving_img=moving_img_ori
        # fixed_img=histogram_matching(fixed_img, moving_img)
        # cv2.imwrite(fr'test_moving\{os.path.basename(i)}',fixed_img)
        alike_or_loft=0
        is_Loftr = False
        if fixed_img.max() <= 10 :
            is_black = True
            ...
            if (feats0['keypoint_scores'] > 0.1).sum().item() <= 10:
                ...
                if (feats0['keypoint_scores'] > 0.8).sum().item() <= 100:
                    ...
                    try:
                        with torch.no_grad():  # 禁用梯度计算
                            feats0 = {'keypoints': feats0['keypoints'] * (fixed_img.shape[0]//512), 'descriptors': feats0['descriptors'],
                                      'keypoint_scores': feats0['keypoint_scores'], 'image_size': feats0['image_size']}
                            feats1 = extractor.run(cv2.resize(moving_img, (512, 512)))
                            feats1 = {'keypoints': feats1['keypoints'] * (fixed_img.shape[0]//512), 'descriptors': feats1['descriptors'],
                                      'keypoint_scores': feats1['keypoint_scores'], 'image_size': feats1['image_size']}
        ...
        if out_path == '':
            out_path = os.path.dirname(
                new_path.replace('crop_moving_img', 'crop_ending_img'))
            out_path1 = os.path.dirname(new_path.replace('crop_moving_img',
                                                                                                      'crop_ending_img_aliked'))
            os.makedirs(out_path1, exist_ok=True)
            ...
            try:
                ...
                if match:
                    ...
                    try:
                        ...
                        merged_image[patch_y + (over_lab[0]-200)//2:end_y + (over_lab[0]-200)//2, patch_x + (over_lab[0]-200)//2:end_x + (over_lab[0]-200)//2] = 0                    # merged_image[patch_y:end_y, patch_x:end_x] = actual_patch
...
def worker_wrapper(dir_path, images_path, HEIGHT, WIDTH, base_name, matcher,window_size,over_lab,config):
    process_directory(dir_path, images_path, HEIGHT, WIDTH, base_name, matcher,window_size,over_lab,config)
...
def main(params):
    ...
    if int(HEIGHT) + int(WIDTH) < 80000:
        max_workers = 4
    if int(HEIGHT) + int(WIDTH) < 160000 and int(HEIGHT) + int(WIDTH) >= 80000:
        max_workers = 3
    ...
    for d in remaining_dirs:
        while len(procs) >= max_workers:
            ...
            for p in procs:
                if not p.is_alive():
                    procs.remove(p)
        p = multiprocessing.Process(target=worker_wrapper, args=(d, images_path, HEIGHT, WIDTH, os.path.basename(first_dir), matcher,window_size,over_lab,config))
        p.start()
        procs.append(p)
