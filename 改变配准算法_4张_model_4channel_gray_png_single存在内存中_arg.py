...
def Aliked_trt(
    ...
    H, mask = cv2.findHomography(mkpts0, mkpts1, cv2.USAC_DEFAULT)
...
def process_subdir(subdir_path, H_dict, HEIGHT, WIDTH, merged_memory, first_dir):
    black_image = np.full((2048, 2048), 255, dtype=np.uint8)
    ...
    for i in H_dict:
        ...
        if H_dict[i] is None:
            # print('H_dict[i] is None')
            registered_color =fixed_img
        else:
            registered_color = cv2.warpPerspective(moving_img, H_dict[i], (moving_img.shape[1], moving_img.shape[0]))
...
def process_directory(dir_path, images_path, HEIGHT, WIDTH, first_dir,matcher):
    ...
    merged_image = np.zeros((HEIGHT, WIDTH), dtype=np.uint8)
    # merged_image = np.full((HEIGHT, WIDTH), dtype=np.uint8, fill_value=127)
    # error_img = np.full((2048, 2048), 50, dtype=np.uint8)
    ...
    for idx, i in enumerate(tqdm.tqdm(images_path)):
        ...
        if not is_black:
            try:
                s=time.time()
                H, flag = Aliked_trt(fixed_img, moving_img, device, matcher=matcher)
