...
def thread_worker(thread_queue, max_w, max_y, output_dir):
    ...
    while True:
        ...
        # for y in range(y, y+h, step_size):
        #     for x in range(x, x+w, step_size):
        #         # 计算截取区域 (防止超出超大图边界)
        #         y_end = min(y + step_size, y+h)
        #         x_end = min(x + step_size, x+w)




        # h_ranges =split_ranges(h_size,5120)
        # w_ranges=split_ranges(w_size,5120)




        # for i in tqdm.tqdm(w_ranges):
        #     for j in h_ranges:
        #         # print(channel)
        #         # print(i,j)
        #         img_np = new_slide.get_img(0, (i, j, i + 20, j + 20), channel)
        #         # print(img_np.shape)
        #         # cv2.imwrite(fr"test\{i}_{j}.png",img_np)
        #         # cv2.imshow("",img_np)
        #         # cv2.waitKey(0)
        #         # img = Image.fromarray(img_np.reshape(img_np.shape[0], img_np.shape[1]))
        #         # img.show()
        #         # time.sleep(100)
        #         new_row = (j - y) // 20
        #         new_col = (i - x) // 20
        #         # full_image.paste(img, (new_col * 256*20, new_row * 256*20))
        #
        #         full_image[new_row*256*20:new_row*256*20+256*20,new_col*256*20:new_col*256*20+256*20]=img_np
        ...
        #                       dtype=np.uint8)
        #
        # for i in tqdm.tqdm(range(x, x + w, 1)):
        #     for j in range(y, y + h, 1):
        #         img_np = new_slide.get_img(0, (i, j, i + 1, j + 1), channel)
        #
        #         # 计算新位置（与原逻辑一致）
        #         new_row = (j - min(range(y, y + h, 1))) // 1
        #         new_col = (i - min(range(x, x + w, 1))) // 1
        #
        #         # 使用OpenCV的ROI操作替代PIL的paste
        #         # 注意：OpenCV是(row,col)顺序，且需要保证ROI区域大小与img_np一致
        #         full_image[new_col * 256: new_col * 256 + img_np.shape[0],
        #         new_row * 256: new_row * 256 + img_np.shape[1]] = img_np.reshape(img_np.shape[0], img_np.shape[1])
        #
        #         # print(p2_val, p1_val)
        #
        ...
        # print(f"Thread processing: {channel} with area {x},{y},{w},{h}")
        # 使用new_slide, p1, p2等参数进行实际处理
...
def main(params):
    ...
    for _ in range(4): process_queue.put(None)
