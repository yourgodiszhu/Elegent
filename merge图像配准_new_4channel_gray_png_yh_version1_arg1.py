...
def auto_register(fixed_path, moving_path, device, extractor, matcher):
    try:
        ...
        # cv2.waitKey(0)
        # cv2.imwrite(r'D:\3d\test_merge\moving_img.png', moving_path)
        ...
        # cv2.imwrite(r'D:\3d\test_merge\fixed.png',fixed_path)
        # #
        # cv2.imwrite(r'D:\3d\test_merge\remove.png',remove_)
        # print(t_x,t_y)
        # 方法二：光流法（适用于非平面透视变换）
        # print(shift)
        # print(H)
...
def process_directory(dir_path, fixed_img, fixed_name):
    ...
    shift, angle, _ = auto_register(fixed_img, moving_path, device, extractor, matcher)
...
Image.MAX_IMAGE_PIXELS = None
def read_pillow_small(img_path, scale=1/10):
        ...
        if i == 'DAPI':
            for j in os.listdir(os.path.join(first_dir, i)):
                if j.endswith('image.png'):
                    ...
                    # cv2.waitKey(0)
                    # cv2.imwrite(r'D:\3d\test_merge\fixed.png', fixed_img)
                    # print(fixed_path)
