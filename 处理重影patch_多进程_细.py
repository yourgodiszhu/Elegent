...
def process_group(g, dir_path, first_dir, subdirs, HEIGHT, WIDTH,
    ...
    try:
        ...
        try:
            ...
            left=False
            right=False
            if start_y>0 and end_y<HEIGHT and start_x>0 and end_x<WIDTH:
                fixed_img=merged_image[start_y:start_y+100, start_x:end_x]
                moving_img=moving_img_end[:100, :]
                os.makedirs(fr'D:\3d\temp_ceshi_name\{os.path.basename(dir_path)}_{g}', exist_ok=True)
                cv2.imwrite(fr'D:\3d\temp_ceshi_name\{os.path.basename(dir_path)}_{g}\{os.path.basename(dir_path)}_{g}_fixed_before.png', fixed_img)
                cv2.imwrite(fr'D:\3d\temp_ceshi_name\{os.path.basename(dir_path)}_{g}\{os.path.basename(dir_path)}_{g}_moving_before.png', moving_img)
                diff = np.abs(fixed_img.astype(np.int16) - moving_img.astype(np.int16))
                cv2.imwrite(fr'D:\3d\temp_ceshi_name\{os.path.basename(dir_path)}_{g}\{os.path.basename(dir_path)}_{g}_diff.png', diff)

                # print(diff.mean())

                if is_mostly_black(diff)==True:
                    top=True
                fixed_img = merged_image[end_y-100:end_y, start_x:end_x]
                moving_img = moving_img_end[-100:, :]
                diff = np.abs(fixed_img.astype(np.int16) - moving_img.astype(np.int16))
                if is_mostly_black(diff)==True:
                    bottom=True
                fixed_img = merged_image[start_y:end_y, start_x:start_x+100]
                moving_img = moving_img_end[:, :100]
                diff = np.abs(fixed_img.astype(np.int16) - moving_img.astype(np.int16))
                if is_mostly_black(diff)==True:
                    left=True
                fixed_img = merged_image[start_y:end_y, end_x-100:end_x]
                moving_img = moving_img_end[:, -100:]
                diff = np.abs(fixed_img.astype(np.int16) - moving_img.astype(np.int16))
                if is_mostly_black(diff)==True:
                    right=True
                if top==True and bottom==True and left==True and right==True:
                    ...
                    print(g,top,bottom,left,right,'有问题')
