...
def auto_register(fixed_path, moving_path, device, extractor, matcher,is_path,save):
    try:
        ...
        if is_path:
            moving_path=read_pillow_small(moving_path)
        else:
            moving_path=read_pillow_small_no_path(moving_path)
        # print(fixed_path.shape)
        # print(moving_path.shape)
        # cv2.imshow('moving_path', moving_path)
        # cv2.waitKey(0)
        # cv_imwrite_unicode(r'D:\3d\test_merge\moving_img.png', moving_path)

        # fixed_path = clahe.apply(fixed_path)
        # moving_path = clahe.apply(moving_path)
        ...
        offsets = pts1[inliers] - pts0[inliers]
        translation = np.median(offsets, axis=0)
        ...
        # t_x *= 20
        # t_y *= 20
        # remove_ = np.roll(moving_path, int(t_y), axis=0)  # 上下移动
        # remove_ = np.roll(remove_, int(t_x), axis=1)  # 左右移动
        # M = np.float32([[1, 0, t_x], [0, 1, t_y]])
        # remove_ = cv2.warpAffine(moving_path, M, (moving_path.shape[1], moving_path.shape[0]))



        shift = [-int(t_y * 20), int(t_x * 20)]
        # shift_=[int(t_y), int(t_x)]
        # remove_=merge_with_offset(moving_path, shift_)
        # if save==True:
        #     remove_=cv2.warpPerspective(moving_path, H, (moving_path.shape[1], moving_path.shape[0]))
        #     cv_imwrite_unicode(r'D:\3d\test_merge\fixed.png',fixed_path)
        #     # #
        #     cv_imwrite_unicode(r'D:\3d\test_merge\remove.png',remove_)
        # print(t_x,t_y)
        # 方法二：光流法（适用于非平面透视变换）
        # print(shift)
        # print(H)
...
def process_subdir(subdir_path, shift, angle):
    ...
    for i in os.listdir(subdir_path):
        ...
        if i.endswith('image.png'):
            print('存在这个图片',i)
            moving_img = universal_imread(os.path.join(subdir_path, i))
            ...
            if abs(angle) > 2:
                ...
                cv_imwrite_unicode(output_path, result_img)
    ...
    # 处理图像
    # img = cv_imread_unicode(moving_path)
    # 进行一些处理
    # cv_imwrite_unicode(moving_path.replace('.png', '_moving.png'), img)
    # 在这里添加你的实际处理逻辑
    # 例如：遍历文件、处理数据等
    # pass
...
def Aliked_trt(
    ...
    # scores_th=0.2,
    # n_limit=20000,
    fixed_path,moving_path,device,extractor,matcher,save=False,
):
    # args = parse_args()
    ...
    # img = cv_imread_unicode(filename)
    ...
    feats0 = {'keypoints': feats0['keypoints'] , 'descriptors': feats0['descriptors'],
    ...
    feats1 = {'keypoints': feats1['keypoints'] , 'descriptors': feats1['descriptors'],
    ...
    kpts0 = feats0["keypoints"].cpu().numpy()  # 形状 (512, 2)
    ...
    pts0 = points0.cpu().numpy().astype(np.float32)  # 匹配点来自你的代码
    pts1 = points1.cpu().numpy().astype(np.float32)
    # H_orig, W_orig = fixed_path.shape[:2]  # 原始图像尺寸
    # H_resized, W_resized = 512, 512  # resize 后的尺寸
    #
    # scale_x = W_orig / W_resized
    # scale_y = H_orig / H_resized
    #
    # # 映射回原始尺寸
    # pts0 = pts0 * np.array([scale_x, scale_y])
    # pts1 = pts1 * np.array([scale_x, scale_y])
    ...
    if save==True:
        print('保存图像')
        print('moving_path',moving_path1.shape)
        print('fixed_path',fixed_path1.shape)
        remove_ = cv2.warpPerspective(moving_path1, H, (moving_path.shape[1], moving_path.shape[0]))
        print(remove_.shape)
        cv_imwrite_unicode(r'D:\3d\test_merge\fixed.png',fixed_path1)
        # #
        cv_imwrite_unicode(r'D:\3d\test_merge\remove.png',remove_)
    t_x, t_y = H[0, 2], H[1, 2]
    shift = [int(t_y), int(t_x)]
...
def process_directory(dir_path, fixed_img, fixed_name):
    ...
    # extractor = TRTInference("aliked-n16512.trt", "aliked-n16", trt.Logger(trt.Logger.VERBOSE))
    ...
    moving_path = os.path.normpath(moving_path)
    # print('moving_img',moving_img.shape)
    moving_img = universal_imread(moving_path)
    # if p1!='HBTT23_4':
    shift, angle, _ = auto_register(fixed_img, moving_path, device, extractor, matcher, is_path=True, save=False)
    # else:
        # shift = [2000, 0]

    # shift, angle, _= Aliked_trt(fixed_img, moving_img, device=device, extractor=extractor, matcher=matcher,save=save)
    print(f'{p1}旋转角度为', angle, '偏移量为', shift)
    ...
    result_img = merge_with_offset(moving_img, shift)
    # shift,angle,_ = auto_register(fixed_img, result_img, device, extractor, matcher,is_path=False,save=save)
    # result_img = merge_with_offset(result_img, shift)
...
Image.MAX_IMAGE_PIXELS = None
def read_pillow_small(img_path, scale=1/20):
    with Image.open(img_path) as img:
        # print(img.mode)
        width, height = img.size
        new_size = (int(width * scale), int(height * scale))
        small_img = img.resize(new_size, Image.LANCZOS)  # 高质量缩小
        return np.array(small_img)  # 返回的是 RGB 格式
def read_pillow_small_no_path(img_path, scale=1/20):
...
def main(params):  # 修改主函数接收参数字典
    ...
    def extract_round_num(name):
        match = re.search(r'第(\d+)轮', name)
        return int(match.group(1)) if match else float('inf')

    # 步骤2：获取A目录下所有子目录，并按轮数排序
    # 获取A目录下所有子目录
    all_subdirs = [os.path.join(A_dir, d) for d in sorted(os.listdir(A_dir),key=extract_round_num)
        ...
        if i == 'DAPI':
            for j in os.listdir(os.path.join(first_dir, i)):
                if j.endswith('image.png'):
                    ...
                    fixed_name = j
                    # print(fixed_name)
                    fixed_img = read_pillow_small(fixed_path)

                    # cv2.imshow('fixed_img', fixed_img)
                    # cv2.waitKey(0)
                    # cv_imwrite_unicode(r'D:\3d\test_merge\fixed.png', fixed_img)
                    # print(fixed_path)
                    # fixed_img = cv_imread_unicode(os.path.normpath(fixed_path), cv2.IMREAD_GRAYSCALE)
                    # print(fixed_path)
    ...
    max_y, max_w = fixed_img.shape[:2]
    max_y=int(max_y)*20
    max_w=int(max_w)*20
    if int(max_y)+int(max_w)<100000:
        max_workers=3
    if int(max_y) + int(max_w) < 160000 and int(max_y) + int(max_w) >= 100000:
        max_workers=2
    if int(max_y)+int(max_w)>=160000:
        max_workers=1
